...
- 体检报告、指标数值、趋势分析等均属于个人敏感健康信息。
- 导出的 ZIP 归档（`/reports/export`）和数据库备份文件包含上述全部数据，下载或传输后请妥善保管，避免通过公共网络或不安全介质分享。
- 本地运行时的 `data/`、`uploads/`、`backups/` 目录仅保存在当前机器，部署到服务器时应配置合适的文件权限与访问控制。
- 上传的 PDF 会在后端进行 MIME 类型与文件头魔数校验，但无法替代安全的终端环境与传输通道（如 HTTPS）。
